...
import ConfiSpec from './ConfiSpec'
import Intro from './Intro'
import ChocoSpec from './ChocoSpec'

const Chocolaterie = () => {

  const chocoRef=useRef()
  const confiRef=useRef()

  function scrollClick() {
    chocoRef.current.scrollIntoView({ behavior: 'smooth' })
}
function scrollClickz() {
  confiRef.current.scrollIntoView({ behavior: 'smooth' })
}
...
<div className='d-flex container-fluid boubou'>
           ...
           Nos chocolats</button>

           <button
           onClick={scrollClickz} 
           className='btn btn-outline-dark'>
           Nos confiseries</button>
        </div>
        ...
        <Intro />
        <img src='https://www.boulangerie-ymare.fr/wp-content/uploads/2019/10/chocolaterie_service3.jpg'
        alt='introBread'
            className='picture'
        />
         
        </div>
        <br ref={chocoRef}  /><br /><br /><br  /> 
        
        <div className='container spec d-flex pl-4'    >
          <ChocoSpec />
        </div>  
        
        <br ref={confiRef}  /><br /><br /><br  /> 
